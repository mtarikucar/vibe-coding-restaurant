...
describe("AuthController", () => {
  ...
  describe("login", () => {
    it("should login a user", async () => {
      ...
      const result = await controller.login(loginDto, req, userAgent, ip);

      expect(service.login).toHaveBeenCalledWith(req.user, userAgent, ip);
      expect(result).toEqual(expectedResult);
    });
  });
  ...
  describe("verifyPasswordResetToken", () => {
    it("should verify a password reset token", async () => {
      ...
      (service.verifyPasswordResetToken as jest.Mock).mockResolvedValue(
        expectedResult
      );

      const result = await controller.verifyPasswordResetToken(
        verifyPasswordResetTokenDto
      );

      expect(service.verifyPasswordResetToken).toHaveBeenCalledWith(
        verifyPasswordResetTokenDto
      );
      expect(result).toEqual(expectedResult);
    });
