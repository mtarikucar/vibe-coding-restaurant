...
import { TableStatus, type Table } from "../../types/table";
import { useTranslation } from "react-i18next";

const EditTableModal: React.FC = () => {
  const { t } = useTranslation();
  const { isEditModalOpen, setEditModalOpen, currentTable, updateTable } =
    useTableStore();
  ...
  useEffect(() => {
    if (currentTable) {
      ...
      setLocation(currentTable.location || "");
    }
  }, [currentTable]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(t("tables.errors.updateFailed"));
    }
  };

  if (!isEditModalOpen || !currentTable) return null;
      ...
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md">
        <div className="flex justify-between items-center p-4 border-b">
          <h2 className="text-xl font-semibold text-gray-800">
            {t("tables.editTable", { number: currentTable.number })}
          </h2>
          <button
